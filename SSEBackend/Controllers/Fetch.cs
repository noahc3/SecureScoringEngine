...
namespace SSEBackend.Controllers
{
    ...
    public class Fetch : ControllerBase
    {
        ...
        [HttpPost("scoringreporttemplate")]
        public ActionResult ScoringReportTemplate([FromBody] GenericEncryptedMessage message) {
            ...
            FileTransferWrapper reportTemplate = Globals.GetScoringReportTemplate(message.TeamUUID, message.RuntimeID);


            byte[] iv;
            byte[] ciphertext = Encryption.EncryptMessage(JsonConvert.SerializeObject(reportTemplate), out iv, message.TeamUUID, message.RuntimeID);
